...
namespace ExercisesAlgo.Trees
{
    public class HotelReviews
    {
        ...
        {
            var words = "qghu_eay_nl_dxfi_cvsc_gg_wk_nqdu_wf_fozv_rtk_pr_p_g_rp_rvys_mwcy_y_c_pev_kef_mz_imkk_svw_r_nzk_cxf_tl_gyp_fad_oo_fxz_co_juv_vabo_gpo_ylf_bnpl_vrvi_ya_yeh_q_qrqp_x_j_loov_o_u_wh_s_cb_coks_zkva_xdkn_yj_h_ixjs_nkk_f_ux_zr_bmnm_q_oke_ly_nk_aug_qrcd_iute_ojw_yyz_vs_m_sa_lfvg_b_aaov_zy_nt_kdcp_s_te_j_hdi_co_zc_fw_qi_tv_wvxh";
            ...
            var list = "a_b_c_a";
            var l2 = new List<string> { "a_b_c", "a_a_a", "a_b_t_a"};
            new HotelReviews().solveLinq(list, l2).Dump();
            new HotelReviews().solve(list, l2).Dump();
        }

        public List<int> solve(string A, List<string> B)
        {
            var trie = new Trie();
            A.Split('_').Distinct().ToList().ForEach(w => trie.Add(w));

            var counts = new List<KeyValuePair<int, int>>();
            for (var i = 0; i < B.Count; i++)
            {
                var count = B[i].Split('_').Where(w => trie.Contains(w)).Count();
                counts.Add(new KeyValuePair<int, int>(i, count));
            }
            return counts.OrderByDescending(c => c.Value).ThenBy(c => c.Key).Select(c => c.Key).ToList();
        }

        public List<int> solveLinq(string A, List<string> B)
        {
            var words = A.Split('_')
                .GroupBy(k=>k)
            ...
            for (var i = 0; i < B.Count; i++)
            {
                var count = B[i].Split('_').Where(e => words.ContainsKey(e)).Count();
                counts.Add(new KeyValuePair<int, int>(i, count));
            }
            return counts.OrderByDescending(c => c.Value).ThenBy(c => c.Key).Select(c => c.Key).ToList();
        }
